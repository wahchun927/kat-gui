...
function ChallengeController($scope,$resource,$location){
    ...
    $scope.goToRegistration=function(challenge_id)
    {
    	...
    	alert(open_challenge_ID);

    	$scope.get_open_challenge = $resource('/jsonapi/list_challenges/');
			$scope.get_open_challenge.get({}, function(response){
				$scope.open_challenge = response;

				console.log($scope.open_challenge.challenges.length);
				for(var i=0; i<$scope.open_challenge.challenges.length;i++){
					if($scope.open_challenge.challenges[i].challenge_id == open_challenge_ID){
						$scope.single_challenge = $scope.open_challenge.challenges[i];
						alert($scope.single_challenge.name);
					}
